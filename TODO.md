...
- [ ] Напрашивается идея добавить dev и prod режимы работы. От них могут зависеть различные оптимизации (напр. переиспользование удаленных колонок)

- [ ] Трансформации которые не трогают данные может перенести в header группу? Например rename только переименовывает, но add уже может добавлять данные в колонке.

- [ ] Переименовать `TransformStep` в `TransformNode`?

- [ ] Каждый узел пребразования должен посылать события о своей работы в общую шину. Такой подход позволит реализовать универсальный механизм сбора статистики, логов и работу с UI.

- [ ] Узел может расчитать внутри себя по умолчанию время ожидания очередного pull. Внутри узла это сделать намного проще. Это применимо только к асинхронным узлам вероятно. На базе этой информации можно расчитать наиболее загруженные узлы и наоборот.

- [ ] Нужно ли разделять узлы на синхронные и асинхронные?

- [ ] Нужно разделить обработку преобразований на две шага (предварительный и обработка данных). На предварительном шаге нужно проверить заголовки, параметры, скомпилировать выражения, тем самым отбрасывая много очевидных ошибок до момента когда данные потекут по flow #dl9adgn1
...
- [ ] Добавить опцию для отключения переиспользования удаленный колонок в `tf.add` так как это неудобно для отладки (меняет органичный порядок колонок). В prod режиме эту опцию можно включать.

- [ ] Подумать над логикой .unroll #djdk52pqq
