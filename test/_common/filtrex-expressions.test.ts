...
test('Filtrex expressions (fields access)', async () => {
...
test('Filtrex expressions (unquoted data field access error)', async () => {
  const tableTransformer = createTableTransformer({
    context: createTestContext(),
    transforms: [
      tf.column.derive({
...
test('Filtrex expressions (unknown symbol access error)', async () => {
  const tableTransformer = createTableTransformer({
  ...
  try {
    ...
    assert.ok(err instanceof TransformStepRowExpressionError)
    assert.equal(err.message, 'Global symbol not found: "foo"')
    err.report()
  }
})
